    ...
    if (!finalOrderId) {
        ...
        if (existingTx) {
            ...
            console.log("Found existing pending transaction:", finalOrderId)
             ...
             if (!finalLeadId) {
                 // Check if profile has referred_by, we don't strictly need lead_id in transaction if profile has it,
                 // but it's good for tracking.
                 // Actually, if we are doing registration payment, the referrer is in profile.referred_by
                 if (profile && profile.referred_by) {
                     // We don't have lead ID from profile directly unless we query it? 
                     // Wait, transaction.lead_id is optional and refers to 'leads' table.
                     // But our affiliate system now uses profiles.referred_by.
                     // So we might NOT need lead_id if we rely on profiles.
                     // However, the webhook logic looks for referrerId.
                     
                     // Let's try to resolve lead_id from leads table if exists, just in case.
                     // But simpler: Ensure transaction doesn't need lead_id if user_id is present.
                 }
             }
