...
import { LogOut, LayoutDashboard, Users, Wallet, ShoppingBag, Menu, X, GraduationCap, Video, Gift, DollarSign, Crown, CreditCard, FileText, Play } from 'lucide-react'
...
export default function DashboardLayout({ children, role }: { children: React.ReactNode, role: 'agent' | 'admin' }) {
  ...
  const navItems = [
    { path: '/dashboard', icon: LayoutDashboard, label: 'Dashboard', roles: ['agent', 'admin'] },
    ...
    { path: '/dashboard/testimonials', icon: Video, label: role === 'admin' ? 'Videos' : 'Testimonials', roles: ['agent', 'admin'] },
    { path: '/dashboard/video-content', icon: Play, label: 'Content Iklan', roles: ['agent'] },
