  ...
  useEffect(() => {
    videoRefs.current.forEach((video, index) => {
      ...
      if (index === activeIndex && isInView) {
        // Try to play
        // FORCE: For the first video, try to mute first if auto-play is strict, 
        // but we want to attempt unmuted if possible.
        // Actually, browsers BLOCK unmuted autoplay unless user interacted.
        // So we MUST start muted often, OR we accept that it might fail and we fallback.
        
        // FIX: Ensure the video property is consistent
        video.muted = isMuted;
        ...
        if (playPromise !== undefined) {
          playPromise.catch(() => {
            // Auto-play policy might block unmuted
             if (!isMuted) {
                 console.log("Autoplay blocked unmuted, switching to muted");
                 video.muted = true;
                 // We don't update isMuted state here to allow user to unmute later
